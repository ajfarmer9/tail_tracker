...
function App() {
  ...
  function EditPlane() {
    ...
    for (let index = 0; index < planes.length; index++) {
      /*Find the plane in the array that matches the tail number sent over store data outside of this loop*/
    ...
    const updatePlane = async () => {
      const planeRef = query(
        collection(db, "tails"),
        where("tailNum", "==", tailNum_T)
      );
      ...
      const updateObjectValues = (id, newValues) => {
        setPlanes((planes) =>
          planes.map((obj) => (obj.id === id ? { ...obj, ...newValues } : obj))
        );
      };
      ...
      <div className="edit_plane_section">
        <div className="editor_container">
          <section className="current_plane_state_section edit_piece">
            ...
            <p className="edit_item">
              Employee's Name: <span>{empName_T}</span>
            </p>

            <p className="edit_item">Washed:</p>
            {washed_T}
            <p className="edit_item">Conditioned:</p>
            {conditioned_T}
          </section>
          ...
          <section className="changed_plane_state_section edit_piece">
            ...
            <div>
              <input
                ...
                type="checkbox"
                name="washer"
                onChange={washerChange}
                checked={washer === "false"}
              />
              False
              <input
                value="true"
              ...
              <input
                type="date"
                id="dateInputWashed"
                name="washer_DateChoose"
                defaultValue={dateWashed_T}
                onChange={(e) => {
                  dateWashed_T = e.target.valueAsDate
                    .toISOString()
                    .slice(0, 10);
                }}
            ...
            <div>
              ...
              <input
                type="date"
                id="dateInputConditioned"
                name="condition_DateChoose"
                defaultValue={dateConditioned_T}
                onChange={(e) => {
                  dateConditioned_T = e.target.valueAsDate
                    .toISOString()
                    .slice(0, 10);
                }}
        ...
        <section className="card_container">
          {planes.map((currentPlane) => (
            <Card
              key={currentPlane.id}
              plane={{
                empName: currentPlane.empName,
                tailNum: currentPlane.tailNum,
                washed: currentPlane.washed,
                bootsCond: currentPlane.bootsCond,
                dateWashed: currentPlane.dateWashed,
                dateCond: currentPlane.dateCond,
              }}
            />
          ))}
        </section>
...
function Card({ plane }) {
  //-------------------------------------CARD--------------------------------------------------------lNum;
  const formattedDateWashed = plane.dateWashed
    ? format(new Date(plane.dateWashed), "MM-dd-yyyy")
    : "";
  const formattedDateCond = plane.dateCond
    ? format(new Date(plane.dateCond), "MM-dd-yyyy")
    : "";

  return (
    <section className="planeCard">
      <section className="plane_id_section">
        <h4 className="two_piece" id="tail_num">
          Tail Number: {plane.tailNum}
        </h4>
        <h4 className="two_piece" id="empName">
          Employee: {plane.empName}
        </h4>
      </section>
      <section className="plane_info_Section">
        <p className="infoPiece">
          <strong>Washed:</strong> {plane.washed ? "True" : "False"}
        </p>
        <p className={plane.washed ? "showDate dateBox" : "hidden"}>
          <strong>Date:</strong> {formattedDateWashed}
        </p>
        <p className="infoPiece">
          <strong>Boots Conditioned:</strong>{" "}
          {plane.bootsCond ? "True" : "False"}
        </p>
        <p className={plane.bootsCond ? "showDate dateBox" : "hidden"}>
          <strong>Date:</strong> {formattedDateCond}
        </p>
        <br />
        <div className="edit_btn_container">
          <button id="edit_btn">
            <Link to="/Edit" state={plane.tailNum} id="edit_btn_link">
              Edit Plane
            </Link>
          </button>
        </div>
